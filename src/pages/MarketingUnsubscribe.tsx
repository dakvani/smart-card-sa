  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (res.ok && data?.valid) { setEmail(data.email ?? null); setStatus("valid"); }
        else if (data?.alreadyUnsubscribed) { setEmail(data.email ?? null); setStatus("already"); }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="w-full max-w-md rounded-2xl border border-border/50 bg-card/60 backdrop-blur-sm p-8 shadow-xl text-center">
        ...
        {status === "validating" && (<><Loader2 className="w-6 h-6 animate-spin mx-auto text-muted-foreground" /><p className="mt-3 text-sm text-muted-foreground">Verifying your link…</p></>)}

        {status === "valid" && (
          <>
            <h1 className="text-2xl font-bold mb-2">Unsubscribe from marketing emails</h1>
            <p className="text-muted-foreground text-sm mb-2">
              {email ? <>Stop sending promotional emails to <span className="text-foreground font-medium">{email}</span>?</> : "Stop receiving promotional emails from us."}
            </p>
            <p className="text-xs text-muted-foreground mb-6">You'll still receive important account emails (security, orders, password resets).</p>
            <Button onClick={confirm} className="w-full">Confirm unsubscribe</Button>
            <Link to="/" className="inline-block mt-4 text-sm text-muted-foreground hover:text-foreground">Back to home</Link>
          </>
        )}

        {status === "submitting" && (<><Loader2 className="w-6 h-6 animate-spin mx-auto text-muted-foreground" /><p className="mt-3 text-sm text-muted-foreground">Updating your preferences…</p></>)}

        {status === "done" && (
          <>
            <CheckCircle2 className="w-10 h-10 text-emerald-500 mx-auto mb-3" />
            <h1 className="text-2xl font-bold mb-2">You're unsubscribed</h1>
            <p className="text-muted-foreground text-sm mb-6">{email ?? "This address"} won't receive promotional emails anymore. Account emails will still be delivered.</p>
        ...
        {status === "already" && (
          <>
            ...
            <p className="text-muted-foreground text-sm mb-6">{email ?? "This address"} is already opted out of marketing.</p>
            <Link to="/"><Button variant="outline" className="w-full">Back to home</Button></Link>
          </>
        )}
        ...
        {(status === "invalid" || status === "error") && (
          <>
            ...
            <p className="text-muted-foreground text-sm mb-6">{errorMsg || "This unsubscribe link is invalid or has expired."}</p>
            <Link to="/"><Button variant="outline" className="w-full">Back to home</Button></Link>
          </>
        )}
      </div>
