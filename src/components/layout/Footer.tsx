...
export function Footer() {
  return (
    ...
    >
      ...
      <div className="container relative z-10 mx-auto px-4 pb-24 pt-16 sm:pb-16 sm:pt-20">
        <div className="grid grid-cols-2 gap-8 sm:gap-10 md:grid-cols-6">
          {/* Brand + contact */}
          <div className="col-span-2 md:col-span-2">
            <Link to="/" className="group inline-flex items-center gap-2.5" aria-label="SmartCard — homepage">
              <SmartCardLogo className="h-7 w-7 text-[#a5b4fc] transition-transform group-hover:scale-110 group-hover:-rotate-3" />
              <span className="font-display text-xl font-bold tracking-tight">
                Smart<span className="text-[#a5b4fc]">Card</span>
              </span>
            </Link>
            <p className="mt-4 max-w-xs font-body-alt text-sm text-white/55">
              Everything you are. In one simple tap.
            </p>
            <ul className="mt-5 space-y-3 font-body-alt text-sm">
              <li className="flex items-start gap-2.5 text-white/70 hover:text-white">
                <Mail className="mt-0.5 h-4 w-4 shrink-0 text-[#a5b4fc]" />
                <a href="mailto:info@smartcardsa.shop" className="break-all">info@smartcardsa.shop</a>
              </li>
              <li className="flex items-start gap-2.5 text-white/70 hover:text-white">
                <Phone className="mt-0.5 h-4 w-4 shrink-0 text-[#a5b4fc]" />
                <a href="tel:+966502900193" dir="ltr">+966 50 290 0193</a>
              </li>
              <li className="flex items-start gap-2.5 text-white/70">
                <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#a5b4fc]" />
          ...
          {Object.entries(footerLinks).map(([category, links]) => (
            <nav key={category} aria-labelledby={`footer-${category.toLowerCase()}`}>
              <h4
                ...
                className="mb-4 font-display text-[11px] font-bold uppercase tracking-[0.2em] text-white/40"
              >
                {category}
              </h4>
              <ul className="space-y-2.5 font-body-alt text-sm" role="list">
                {links.map((link) => (
                  <li key={link.name}>
                    <Link
                      to={link.href}
        ...
        {/* Bottom bar */}
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/[0.06] pt-6 text-center sm:mt-16 sm:pt-8 md:flex-row md:text-left">
          <p className="font-body-alt text-xs text-white/40">
              ...
              {[
                ...
                <li key={s.label}>
                  <a
                    href="#"
                    aria-label={s.label}
                    className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] text-white/60 transition-all hover:border-[#4f46e5]/50 hover:bg-[#4f46e5]/10 hover:text-[#a5b4fc]"
                  >
